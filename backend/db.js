...
export const initializeDatabase = () => {
  ...
  db.connect((err) => {
    ...
    // 删除 todos 表（如果存在）
    db.query(dropTodosTable, (err) => {
      if (err) {
        console.error("删除 todos 表失败: " + err.message);
        db.end(); // 关闭连接
        return;
      }
      console.log("todos 表已删除（如果存在）");

      // 删除 users 表（如果存在）
      db.query(dropUsersTable, (err) => {
        if (err) {
          console.error("删除 users 表失败: " + err.message);
          db.end(); // 关闭连接
          return;
        }
        console.log("users 表已删除（如果存在）");
